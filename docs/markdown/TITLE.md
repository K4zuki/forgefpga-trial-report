...
本家の通り[OSL641501-ARA(高輝度赤発光・カソードコモン)](#matrixled-akizuki)を使いました。よくあるピン配置です。
...
ciniml氏が設計・頒布しているボードです。

コモン側に電流ドライバがない、シンプルな回路になっています。コモン側IOの出力電流が定格を超えないように制限抵抗を選択する必要があります。
そのかわり、アノードコモンでもカソードコモンでも使えます。今回のようにカソードコモンタイプを選択した場合ではROW出力をLoにします。
筆者は制限抵抗に手持ちの2.2kを使ったので暗めですが、SLG47910のIOを痛めたくないのでまあ良しとしています。
...
[@tbl:io-plan-csv]は加工済みCSVファイルを元に表にしたものです。"タイル"の座標、機能名、FPGA回路の信号名を突き合わせる内容になっています。
